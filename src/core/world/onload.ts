...
export function applyOnload(world: World, onload?: string[]): World {
  ...
  const RUR = {
    ...
    add_object: (kind: string, x: number, y: number, opts?: any) => {
      const ix = Math.floor(x), iy = Math.floor(y);
      const k = normalizeObjectKind(String(kind));
      if (!k) {
    ...
    // Goal: final robot position (image is decorative and ignored)
    add_final_position: (_image: string, x: number, y: number) => {
      const ix = Math.floor(x), iy = Math.floor(y);
      if (ix < 1 || iy < 1) return;
      dynGoal.finalPositions ??= [];
      if (!dynGoal.finalPositions.some(([px, py]) => px === ix && py === iy)) {
        dynGoal.finalPositions.push([ix, iy]);
      }
    },
    ...
    // Scratch space some worlds use to stash their own data (e.g. a path array)
    ...
    frame_insertion: (_name: any, _arg: any) => {},
    _write_ln: (_text: any) => {},
    _move_: () => {}
  } as any;
  ...
  if (dynGoal.finalPositions && dynGoal.finalPositions.length > 0) {
    (w as any).goal = (w as any).goal || {};
    const prev = (w as any).goal.possible_final_positions ?? [];
    (w as any).goal.possible_final_positions = [...prev, ...dynGoal.finalPositions];
  }
